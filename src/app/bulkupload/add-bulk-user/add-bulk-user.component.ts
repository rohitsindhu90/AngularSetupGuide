...
import { AddBulkUserValidateViewModel } from 'src/app/_models/bulk-upload/addbulkuservalidateviewmodel';
...
export class AddBulkUserComponent {
  ...
  model: AddBulkUserValidateViewModel;
  modelCopy: AddBulkUserValidateViewModel;
  csvfilename: string;
  ...
  ngOnInit() {
    this.model = new AddBulkUserValidateViewModel();
    this.modelCopy = new AddBulkUserValidateViewModel();
    this.files = null;
  ...
  upload() {
    ...
    let url = this.userService.getBulkAddUserUploadUrl();
  ...
  validateFileOnServer(batchId: string) {
    ...
    if (!batchId) {
      ...
      this.userService.validateHeaderAddBulkUserFiles(batchId).then(data => {
        this.blockedPanel = false;
        if (data.item1 === false) {
  ...
  insertInRawTable(batchId: string) {
    ...
    this.userService.insertInAddBulkUserRawTable(batchId).then((data) => {
      this.blockedPanel = false;
      if (data.item1 === false) {
        this.processingMessage = data.item2;
        this.clear();
      ...
      this.blockedPanel = true;
      this.processingMessage = "Files header validate done! Please wait while we insert data in raw table and validate file data!";
      this.validateAddBulkUserData(batchId);
    });
  }

  validateAddBulkUserData(batchId: string) {

    this.userService.validateAddBulkUserData(batchId).then((data) => {
      this.blockedPanel = false;
      this.model = data;
      this.clear();
      this.processingMessage = "";
  ...
  hasValidRecord(): boolean {
    if (this.modelCopy.adduserrawviewmodellst) {
      return (this.modelCopy.adduserrawviewmodellst.filter((val, index) => {
  ...
  onTabChange(e: any) {
    ...
    if (this.model.batchid) {
      ...
      if (e.index == 0) {
        this.model.adduserrawviewmodellst = this.modelCopy.adduserrawviewmodellst.filter((val, index) => {
          return val.errormessage != null;
        });
        this.csvfilename = "AddBulkUser-InvalidData";
      }

      else if (e.index == 1) {
        this.csvfilename = "AddBulkUser-ValidData";
        this.model.adduserrawviewmodellst = this.modelCopy.adduserrawviewmodellst.filter((val, index) => {
          return val.errormessage == "" || val.errormessage == null || val.errormessage == undefined;
        })
      }
      else if (e.index == 2) {
        this.csvfilename = "AddBulkUser-AllData";
        this.model.adduserrawviewmodellst = this.modelCopy.adduserrawviewmodellst.slice();
      }
    }
  }

  DownloadFile(): void {
    this.userService.GetAddBulkUserProtoType().subscribe(response => {
      var fileName = "Add User Template.xlsx";
  ...
  saveValidateUser() {
    ...
    this.loader.emit(this.userService.InsertBulkUserDataInUser(this.model.batchid).subscribe((result: any) => {
      if (result) {
