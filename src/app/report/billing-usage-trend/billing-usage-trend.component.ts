...
@Component({
    selector: 'app-billing-usage-trend',
    templateUrl: './billing-usage-trend.component.html'
})
/** billing-usage-trend component*/
export class BillingUsageTrendComponent implements OnInit {
    ...
    onPageLoad() {
        let process1 = this.loadBillingUsageTrendReport();
    ...
    loadInvoiceMonthsNetworkCC(): Promise<any> {
        ...
        return this.invoiceDateService.getInvoiceMonth().then((data) => {
            if (data && data.length) {
                ...
                this.csvfilename = "BillingUsageTrend_" + this.invoicemonthArray.find(x => x.value == this.fromdate).label + "_" + this.invoicemonthArray.find(x => x.value == this.todate).label;
                this.onPageLoad();
            }
            else {
...
*/
    ...
    onInvoiceMonthChange() {
        ...
        if (this.error) {
            ...
            this.loader.emit(this.loadBillingUsageTrendReport());
            this.csvfilename = "BillingUsageTrend_" + this.invoicemonthArray.find(x => x.value == this.fromdate).label + "_" + this.invoicemonthArray.find(x => x.value == this.todate).label;
        }
    }
...
*/
    ...
    //Binding Grid Data
    loadBillingUsageTrendReport(): Promise<any> {

        let dynamicWidth: { id: number, width: string }[] = [{ id: 1, width: "410px" },
        { id: 2, width: "825px" },
        { id: 3, width: "950px" }]

        return this.invoicereportservice.GetBillingUsageTrendReport(this.fromdate, this.todate, this.networkguid, this.billingplatformguid, this.benID, this.banID, this.reportinggroup1ID, this.reportinggroup2ID
            , this.reportinggroup3ID, this.reportinggroup4ID, this.reportinggroup5ID, this.reportinggroup6ID).then(res => {
                ...
                this.gridOuterWidth = dynamicWidth.find(a => a.id == invoiceMonthDiff).width;
            });
    }
